---
title: "小调的主音不是6!"
description: "请坚持用1作为任何音阶的主音"
date: 2025-05-20
draft: false
tags:
  - post
  - thoughts
  - musicTheory
---

> [!note] 要改变人的思维惯性是很难的一件事情。

---
# 思维惯性
也许我们不谈到底要用6还是1来表示小调主音，我们来聊聊人的思维惯性。

美国至今还在使用Imperial unit。 比如miles, lbs, foot and inches. 当几乎全世界都在使用千米，千克的标准单位时，美国说，我们就这样吧，因为这些单位存在于美国社会的各处，螺丝刀，体重秤，吉他的琴弦长度，改起来太难了，就不改了吧。

英国， 日本等地汽车靠左行驶。

部分书籍延续了从上到下，从右至左的书写阅读顺序。即使当代，一些日本或台湾香港出版社仍会出版这样的书籍。

>[!quote]
> Proactive inhibition—or interference, as it is most commonly known these days—is a well-documented phenomenon in the brain where old knowledge can prevent us from learning new knowledge if that new knowledge conflicts with the old knowledge.  
>Molly Gebrian, _Learn Faster, Perform Better_, 1st ed. (Oxford University PressNew York, 2024), [https://doi.org/10.1093/oso/9780197680063.003.0004](https://doi.org/10.1093/oso/9780197680063.003.0004).

---
# 小调为什么会是6？
要厘清这一切争执的源头，似乎要从解释清楚小调以6开头的规则开始比较好。这种思维方式是这样的：**如果大调以1开始，排列出1-2-3-4-5-6-7，那么小调，这里专门指[[Glossary#Natural Minor 自然小调|自然小调]]，就以6级开始排列，6-7-1-2-3-4-5**

而另外一方的观点则是：**任何调式都是以1开始**， 那么[[Glossary#Natural Minor 自然小调|自然小调]] 同样应该排列为：1-2-♭3-4-5-♭6-♭7  

为什么有人要以6作为小调开始呢？首先它不会出现任何升降号，在这一程度上，它相对简易一些，其次，在视唱和听觉训练当中，它似乎也要比另外一种方式简单。

# 缺陷？

## 逻辑的不自洽
首要的缺陷就是逻辑的缺失，在以级数思维思考的情况下，每一个级数在数字背后都有它蕴含的对应的**听感**。比如1级，不管在任何情况下都有一种“主” 一种“tonic”的感受，比如3级，如果是大三度，它在任何时候都应该有大三度的色彩。而我们反观6-7-1-2-3-4-5的排列，3级在哪？3级或者那个叫“mi”的音级实际上是在5级。

## 音乐素材的缺失
你以为6-7-1-2-3-4-5似乎是一种捷径，但它其实大大限制了音乐素材的拓展。不信的话，我们试着以这种逻辑排列一下[[Glossary#Harmonic Minor 和声小调|和声小调]]，即升高7音，或者在这个逻辑下升高5： 6-7-1-2-3-4-♯5， 或者再试一下[[Glossary#Melodic Minor 旋律小调|旋律小调]]， 在这个逻辑下再升高4: 6-7-1-2-3-♯4-♯5。  
看出问题了吗？本来以6开头是为了避开升降号，然而，随着音乐素材的扩充，这种思维方式不但没有解决升降号，反而引来了更大的麻烦：概念的重复替换。本来“♯4“这个概念是贯穿音乐逻辑的一个“Lydian”声音效果，然而在这里已经变成了不知道是什么的东西。所以以6开头，大大限制了你对于音乐素材的延伸，而限制在了自然大调的语境当中。

---
中国的音乐教育有着它quirky的一面，比如[[FixedorMoveable#solfège（唱名）? Fixed-do System and Movable-do System|首调和固定调的混用]]， 再比如把小调主音当作6。事实上人类历史上似乎不怎么存在一种完美的解决方案？想想看手动挡的汽车，也许一个“完美”的系统应该是需要一个加速一个刹车踏板，难以想象人为了运行必须要依照机械的逻辑需要踩离合换挡，并按照这种逻辑设计汽车运行了100多年 ——我爱手动挡，但就算是自动挡，还是有挡位存在。再比如代码，也是人设计出来的一种交互语言，同样也有它设计出来的理由——为了解决某种需求，但人始终在不断发明新的语言来满足日益增长的需求。

所以在音乐上，我们也可以总结出：小调作为6在历史上具有它一定的设计意义：他统一了调式——以一种调式来概括7条中古调式。他简易了视唱的难度。但在不断发展的现代音乐体系中，它不能够满足音乐人对于其他非大调调式，调式音阶的思维逻辑。所以在当代音乐体系中，不断证实了它的不合理性。所以音乐人需要一种新的逻辑来帮助人理解音乐。这就有了以1确定任何调式音阶的思维方式。

# 还有意义吗？
事实上，把小调当作6的思维方式在即兴思考当中是有明确的意义的，但是我们需要再重新转换一下思维方式。
首先我们要把小调当作6的思维方式扩展到整个音阶体系当中，总结为：它其实以一种[[Glossary#Parent Scale|Parent Scale]]来主导其下所有子音阶的思维方式。既然它可以把小调当作6，那么同样的，它也可以把[[Glossary#Dorian|Dorian]]当作以2开始的音阶：2-3-4-5-6-7-1.  
再比如，在一套[[Glossary#ii - V - I|ii-V-I]]进行当中，只用主调的音阶思考，而不去考虑 ii [[Glossary#Dorian|Dorian]]或者 V [[Glossary#Mixolydian|Mixolydian]] 。这样的思考方式是最安全，也是在部分曲目当中符合作曲家逻辑的思考方式。

关于平行和关系调式的思维方式，其实也主导了整个吉他学习的重点，理解了其中的奥秘，就能大大提升你对音乐的认知。详细的解释请阅读：[[Parallel vs Relative]]。




---
<form
  action="https://buttondown.com/api/emails/embed-subscribe/RunshengZhao"
  method="post"
  target="popupwindow"
  onsubmit="window.open('https://buttondown.com/RunshengZhao', 'popupwindow')"
  class="embeddable-buttondown-form"
  style="max-width: 600px; font-family: sans-serif; margin: 0 auto; text-align: center;"
>
  <div style="margin-bottom: 1em;">
    <span style="font-size:1.1em; font-weight:bold;">Subscribe 订阅我的数字花园！</span><br />
    (I won't spam you — you'll be lucky if I get my lazy ass up to write something.)
  </div>

  <input
    type="email"
    name="email"
    id="bd-email"
    placeholder="Your email here..."
    required
    style="width: 80%; padding: 0.5em; font-size: 1em; margin-bottom: 0.25em; text-align: center;"
  /><br />

  <input
    type="submit"
    value="Subscribe|订阅!"
    style="padding: 0.5em 1em; font-size: 1em; background-color: #333; color: white; border: none; border-radius: 4px; cursor: pointer;"
  />
</form>

---
> [!note] I’d love to hear what you think. Use your GitHub account to leave a comment below!  欢迎用 GitHub 账号在下方留言交流!

<script src="https://giscus.app/client.js"
        data-repo="RunshengZhao/rz"
        data-repo-id="R_kgDOOrPyUA"
        data-category="Announcements"
        data-category-id="DIC_kwDOOrPyUM4CqW1I"
        data-mapping="pathname"
        data-strict="0"
        data-reactions-enabled="1"
        data-emit-metadata="0"
        data-input-position="top"
        data-theme="preferred_color_scheme"
        data-lang="en"
        data-loading="lazy"
        crossorigin="anonymous"
        async>
</script>